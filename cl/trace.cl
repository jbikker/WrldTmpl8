...
#define RAY_T td.w
#define CLAMP(f,a,b) (fmax((a),fmin((f),(b))))
...
)
{
	...
	float4 tm = ((float4)((up >> 20) + ((bits >> 5) & 1), ((up >> 10) & 31) + ((bits >> 13) & 1),
		(up & 31) + ((bits >> 21) & 1), 0) - A * 0.03125f) * rV;
	float t = 0;
	...
	do
	{
		...
		if (o)
		{
			...
			tm = (convert_float4( (uint4)((tp >> 20) + ((bits >> 5) & 1), ((tp >> 10) & 127) + ((bits >> 13) & 1),
			...
			do
			{
				...
				else // brick
				{
					...
					tm = (convert_float4( (uint4)((p >> 20) + OFFS_X, ((p >> 10) & 1023) + OFFS_Y, (p & 1023) + OFFS_Z, 0) ) - A) * rV;
					...
					{
					#if MORTONBRICKS == 1
						uint v = o + Morton3( (p >> 20) + ((p >> 7) & (BMSK * BRICKDIM)) + (p & BMSK) * BDIM2 );
					#else
						...
						if (t == tm.y) tm.y += td.y, p += ((bits << 2) & 3072) - 1024, last = 1;
				...
				if (t == tm.y) tm.y += td.y, tp += DIR_Y << 10, last = 1;
				if (t == tm.z) tm.z += td.z, tp += DIR_Z, last = 2;
			} while ((tp & UBERMASK3) == tq);
			// restore ubergrid traversal state
			tm = tm_;
		}
		t = min( tm.x, min( tm.y, tm.z ) );
		if (t == tm.x) tm.x += td.x, up += DIR_X << 20, last = 0;
		if (t == tm.y) tm.y += td.y, up += DIR_Y << 10, last = 1;
		if (t == tm.z) tm.z += td.z, up += DIR_Z, last = 2;
	} while (!(up & 0xfe0f83e0));
	return 0U;
}
...
)
{
	...
	do
	{
		...
		else if ((o & 1) == 1) /* non-empty brick */
		{
			...
			const float4 tm_ = tm;
			// intialize brick traversal
			tm = A + V * (RAY_T *= 8); // abusing tm for I to save registers
			...
			do // traverse brick
			{
				...
				RAY_T = min( tm.x, min( tm.y, tm.z ) );
				if (RAY_T == tm.x) tm.x += td.x, p += DIR_X << 20, last = 0;
				else if (RAY_T == tm.y) tm.y += td.y, p += ((bits << 2) & 3072) - 1024, last = 1;
				else if (RAY_T == tm.z) tm.z += td.z, p += DIR_Z, last = 2;
			} while (!(p & TOPMASK3));
			tm = tm_; // restore top-grid traversal state
		}
		RAY_T = min( tm.x, min( tm.y, tm.z ) );
		if (RAY_T == tm.x) tm.x += td.x, tp += DIR_X << 20, last = 0;
		else if (RAY_T == tm.y) tm.y += td.y, tp += DIR_Y << 10, last = 1;
		else if (RAY_T == tm.z) tm.z += td.z, tp += DIR_Z, last = 2;
	} while (!(tp & 0xf80e0380));
}